...
function getCategoryAndExt(name: string, mimeOrExt?: string) {
  ...
  if (typeLower === 'pdf' || nameExt === 'pdf' || typeLower.includes('pdf')) {
    return { category: 'PDF', ext: 'PDF', bg: 'bg-gradient-to-br from-rose-50 to-rose-100/50', color: 'text-rose-500' };
  }
  if (['zip', 'rar', 'tar', 'gz', '7z'].includes(nameExt) || typeLower.includes('zip') || typeLower.includes('archive') || typeLower.includes('compressed')) {
    return { category: 'Archive', ext: nameExt ? nameExt.toUpperCase() : 'ZIP', bg: 'bg-gradient-to-br from-emerald-50 to-emerald-100/50', color: 'text-emerald-500' };
  }
  if (nameExt === 'fig' || typeLower === 'figma' || typeLower.includes('fig')) {
    return { category: 'Figma', ext: 'FIG', bg: 'bg-gradient-to-br from-fuchsia-50 to-fuchsia-100/50', color: 'text-fuchsia-500' };
  }
  if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg'].includes(nameExt) || typeLower.startsWith('image/')) {
    ...
    return { category: 'Image', ext: ext.length > 4 ? ext.substring(0, 3) : ext, bg: 'bg-gradient-to-br from-blue-50 to-blue-100/50', color: 'text-blue-500' };
  }
  if (['mp3', 'wav', 'm4a', 'ogg', 'flac'].includes(nameExt) || typeLower.startsWith('audio/')) {
    return { category: 'Audio', ext: (nameExt || 'AUD').toUpperCase(), bg: 'bg-gradient-to-br from-violet-50 to-violet-100/50', color: 'text-violet-500' };
  }
  if (['doc', 'docx', 'txt', 'rtf', 'odt', 'md'].includes(nameExt) || typeLower.includes('word') || typeLower.includes('document') || typeLower.includes('text')) {
    return { category: 'Document', ext: (nameExt || 'DOC').toUpperCase(), bg: 'bg-gradient-to-br from-indigo-50 to-indigo-100/50', color: 'text-indigo-500' };
  }
  return {
    category: nameExt ? nameExt.toUpperCase() : "Document",
    ext: (nameExt ? nameExt.substring(0, 3).toUpperCase() : "DOC"),
    bg: 'bg-gradient-to-br from-gray-50 to-gray-100/50', color: 'text-gray-500'
  };
}
    ...
    if (filterModified) {
      result = result.filter(file => {
        ...
        if (filterModified === 'today') {
          return fileDate.getDate() === nowDate.getDate() && 
          ...
          <div>
            <div className="flex items-baseline gap-3">
              ...
              <div className="text-[13px] text-gray-400 font-semibold flex items-center gap-2">
                Project Alpha <span className="text-gray-300 font-normal">/</span> Marketing <span className="text-gray-300 font-normal">/</span> <span className="font-bold text-gray-900">Cloud Storage</span>
              </div>
            </div>
          </div>
          ...
          <div className="w-full xl:w-[320px] shrink-0 xl:mt-0">
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="text-[13px] font-bold text-gray-900">Cloud Storage</div>
