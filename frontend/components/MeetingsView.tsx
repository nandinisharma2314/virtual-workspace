...
export default function MeetingsView() {
  ...
  const [editMeetingForm, setEditMeetingForm] = useState({ id: 0, title: "", description: "", date: "", startTime: "", endTime: "" });
  ...
  const openEditMeeting = (meeting: any) => {
    const sDate = new Date(meeting.rawStartTime);
    const eDate = new Date(meeting.rawEndTime);
    ...
    <div className="flex w-full h-full min-h-0 flex-col bg-[#FAFBFC] overflow-hidden relative selection:bg-indigo-500/20">
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <div className="p-8 mx-auto max-w-[1400px] space-y-10 relative z-10">
          ...
          {/* Schedule */}
          <section className="bg-gradient-to-br from-indigo-50/40 via-white to-purple-50/40 p-8 rounded-[32px] border border-indigo-100/60 shadow-sm relative overflow-hidden">
            <div className="absolute -top-24 -right-24 w-64 h-64 bg-purple-300/20 rounded-full blur-[80px] pointer-events-none"></div>
            <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-indigo-300/20 rounded-full blur-[80px] pointer-events-none"></div>
            ...
            <div className="relative z-10 mb-8 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-[20px] font-bold text-gray-900 tracking-tight">Today's Schedule</h2>
                  <p className="text-[13px] font-medium text-gray-500">Your upcoming events and syncs</p>
                </div>
              </div>
              <button 
                onClick={() => router.push('/calendar')}
                className="flex items-center gap-1.5 rounded-xl bg-white px-4 py-2 text-[14px] font-bold text-indigo-600 shadow-sm border border-indigo-50 hover:bg-indigo-50 transition-colors"
              >
              ...
              {filteredUpcoming.map((meeting) => (
                <motion.div
                  ...
                  className={`group relative flex flex-col justify-between overflow-hidden rounded-[24px] border p-6 transition-all duration-300 ${
                    meeting.isNow
                      ? "border-indigo-300 bg-gradient-to-br from-indigo-100 to-indigo-50 shadow-[0_8px_30px_rgba(79,70,229,0.15)]"
                      : "border-purple-100 bg-gradient-to-br from-purple-50/80 to-pink-50/50 hover:border-purple-300 hover:shadow-[0_12px_40px_rgba(168,85,247,0.12)]"
                  }`}
                >
                  <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-purple-400 via-pink-400 to-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                  <div>
                            ...
                            >
                              {currentUser?.role === "Admin" ? (
                                <>
                                  ...
                                  <button onClick={() => handleDeleteMeeting(meeting.id)} className="w-full text-left px-4 py-2 text-[13px] font-medium text-red-600 hover:bg-red-50 transition-colors">
                                    Delete
                                  </button>
                                </>
                              ) : (
                                <button className="w-full text-left px-4 py-2 text-[13px] font-medium text-gray-500 cursor-not-allowed">
                                  No Actions
                                </button>
                              )}
                            </motion.div>
        ...
        {showEditMeetingModal && (
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-gray-100 px-8 py-5 bg-gray-50/50">
                <h3 className="text-[18px] font-bold text-gray-900">Edit Meeting</h3>
                <button 
              ...
              <form onSubmit={handleUpdateMeeting} className="p-8 space-y-5">
                <div>
                  <label className="mb-2 block text-[13px] font-bold text-gray-700">Meeting Title</label>
                  <input
                    type="text"
