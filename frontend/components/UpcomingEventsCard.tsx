...
export default function UpcomingEventsCard({ upcomingEventsCard = [] }: { upcomingEventsCard?: any[] }) {
  return (
    <div className="w-full h-full flex-1 flex flex-col justify-between rounded-xl border border-gray-200/80 bg-white p-3.5 shadow-2xs overflow-hidden min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-hidden flex flex-col justify-center">
        {upcomingEventsCard.length > 0 ? (
          <ul className="space-y-2 overflow-hidden flex flex-col justify-around">
            {upcomingEventsCard.map((e, i) => {
              ...
              return (
                <li key={`${e.title}-${i}`} className="flex items-center gap-3 rounded-xl p-1 hover:bg-gray-50/80 transition-colors min-w-0">
                  <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${e.bg || "bg-indigo-50"} ${e.color || "text-indigo-600"} shadow-2xs`}>
          ...
          <p className="text-center text-[12px] text-gray-400 py-3">No upcoming events</p>
        )}
      </div>
    </div>
  );
