...
=======
  ...
  // Assignee counts
  const assigneesMap: Record<string, number> = {};
  tasks.forEach(t => {
    const name = t.assigneeName || "Unassigned";
    assigneesMap[name] = (assigneesMap[name] || 0) + 1;
  });
  const topAssignees = Object.entries(assigneesMap)
    .map(([name, count]) => ({ name, count, width: `${totalTasks ? Math.round((count / totalTasks) * 100) : 0}%` }))
    .sort((a, b) => b.count - a.count)
    .slice(0, 5);
...
=======
      ...
      <div className="grid grid-cols-4 gap-4 mb-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-200/80 p-4 shadow-2xs flex flex-col justify-between">
          <span className="text-[12px] font-bold text-gray-500 mb-2">Completed</span>
          <div className="flex items-end justify-between">
            <span className="text-2xl font-black text-gray-900">{doneTasks}</span>
            <span className="text-[11px] font-extrabold text-emerald-500 bg-emerald-50 px-1.5 py-0.5 rounded">{donePct}%</span>
          </div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-200/80 p-4 shadow-2xs flex flex-col justify-between">
          <span className="text-[12px] font-bold text-gray-500 mb-2">In Progress</span>
          <div className="flex items-end justify-between">
            <span className="text-2xl font-black text-gray-900">{inProgressTasks}</span>
            <span className="text-[11px] font-extrabold text-blue-500 bg-blue-50 px-1.5 py-0.5 rounded">{inProgressPct}%</span>
          </div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-200/80 p-4 shadow-2xs flex flex-col justify-between">
          <span className="text-[12px] font-bold text-gray-500 mb-2">To Do</span>
          <div className="flex items-end justify-between">
            <span className="text-2xl font-black text-gray-900">{todoTasks}</span>
            <span className="text-[11px] font-extrabold text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded">{todoPct}%</span>
          </div>
        </div>
      </div>
            ...
            <div className="flex-1 flex items-center justify-center gap-8">
              ...
              <div className="relative w-32 h-32 rounded-full border-[12px] border-emerald-500 flex items-center justify-center" style={{ borderRightColor: '#3B82F6', borderBottomColor: '#3B82F6', borderLeftColor: '#D1D5DB' }}>
                <div className="flex flex-col items-center">
                  <span className="text-xl font-black text-gray-900 leading-none">{totalTasks}</span>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[12px] font-bold text-gray-900">{doneTasks} <span className="text-gray-400 font-medium">({donePct}%)</span></span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-blue-500 shrink-0"></span>
                  <span className="text-[12px] font-semibold text-gray-600 w-20">In Progress</span>
                  <span className="text-[12px] font-bold text-gray-900">{inProgressTasks} <span className="text-gray-400 font-medium">({inProgressPct}%)</span></span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[12px] font-bold text-gray-900">{todoTasks} <span className="text-gray-400 font-medium">({todoPct}%)</span></span>
>>>>>>> Stashed changes
                </div>
              </div>
            </div>
...
=======
          ...
          <div className="bg-white rounded-2xl border border-gray-200/80 p-5 shadow-2xs h-64 flex flex-col">
            ...
            <div className="flex-1 flex flex-col justify-start gap-3 pt-2 overflow-y-auto">
              {topAssignees.length > 0 ? topAssignees.map((item, i) => (
...
=======
                ...
                <span className="text-sm text-gray-400 py-4 text-center">No tasks assigned yet.</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-200/80 p-5 shadow-2xs h-64 flex flex-col">
            ...
            <div className="flex-1 flex items-end justify-center gap-10 pb-4">
              <div className="flex flex-col items-center gap-2">
                <span className="text-[11px] font-bold text-gray-500">{highPriority}</span>
                <div className="w-10 bg-rose-500 rounded-t-lg transition-all" style={{ height: `${(highPriority / maxPriority) * 120}px`, minHeight: highPriority ? '10px' : '0' }}></div>
                <span className="text-[11.5px] font-semibold text-gray-600">High</span>
              </div>
              <div className="flex flex-col items-center gap-2">
                <span className="text-[11px] font-bold text-gray-500">{mediumPriority}</span>
                <div className="w-10 bg-amber-500 rounded-t-lg transition-all" style={{ height: `${(mediumPriority / maxPriority) * 120}px`, minHeight: mediumPriority ? '10px' : '0' }}></div>
                <span className="text-[11.5px] font-semibold text-gray-600">Medium</span>
              </div>
              <div className="flex flex-col items-center gap-2">
                <span className="text-[11px] font-bold text-gray-500">{lowPriority}</span>
                <div className="w-10 bg-emerald-500 rounded-t-lg transition-all" style={{ height: `${(lowPriority / maxPriority) * 120}px`, minHeight: lowPriority ? '10px' : '0' }}></div>
                <span className="text-[11.5px] font-semibold text-gray-600">Low</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200/80 p-5 shadow-2xs h-64 flex flex-col">
            ...
            <div className="flex-1 flex items-center justify-center text-center text-gray-400 text-sm">
              <p>Not enough historical data<br/>for this project yet.</p>
>>>>>>> Stashed changes
            </div>
          </>
        )}
